...
PROMPT_MAP = {
    ...
    "거미": "spider", "[무량공처]": "Satoru Gojo", "벽돌": "brick", "탄산음료": "soda"
}
...
class PaintGame:
    ...
    def update_tutorial_ui(self):
        ...
        # 2. 버튼 보이기/숨기기 및 위치 처리 (중앙 하단 정렬)

        # 이전 버튼 처리 (중앙 기준 왼쪽: relx=0.45)
        if self.current_tut_page == 0:
            self.prev_btn.place_forget()
        else:
            self.prev_btn.place(relx=0.9, rely=0.9, anchor="center")

        # 다음 버튼 처리 (중앙 기준 오른쪽: relx=0.55)
        if self.current_tut_page == len(self.tutorial_images) - 1:
            self.next_btn.place_forget()
        else:
    ...
    def calculate_ai_score(self, target_word_kr):
        if self.model is None:
            # AI 모델이 없을 경우: 점수와 text_prob (0.0)을 튜플로 반환
            return random.randint(30, 70), 0.0

        target_word_en = PROMPT_MAP.get(target_word_kr, "object")
        text_prompts = [
            f"a sketch of a {target_word_en}",
            f"written word '{target_word_en}'",
            "written text",
            "text or writing"
        ]
        ...
        score_float = probs[0].item()
        score_word = probs[1].item()
         # 글씨 확률 추출
        print("text_prob: ", score_word * 100)
        print("score_float: ", score_float * 100)

        # 점수 계산 로직: 제시어 확률 * 100 - 글씨 확률 * 70
        final_score = int(score_float * 100 - score_word * 100)

        # 점수 보정 (최소 10점, 최대 98점)
        if final_score < 10: final_score = 10
        # 100점 방지를 위해 98점으로 제한 (원본 코드와 일치시키기 위해 100으로 유지)
        if final_score > 98: final_score = 100

        # 최종 점수와 글씨 확률을 튜플 형태로 반환
        return final_score, score_word

    def check_answer(self):  # <--- 인자 (text_prob) 제거
        self.check_button.config(text="채점 중...", state="disabled")
        self.root.update()

        answer_kr = self.prompts[self.current_prompt_index]

        # 수정된 calculate_ai_score를 호출하여 score와 text_prob을 동시에 받습니다.
        score, word  = self.calculate_ai_score(answer_kr)
        ...
        if self.current_prompt_index >= len(self.prompts):
            # 게임 종료 시, 다시 시작할지 묻는 메시지 박스 표시 (askyesno로 변경)
            play_again = messagebox.askyesno(
                title="게임 종료!",
                message="모든 준비물을 만디에게 보여줬다요! 게임을 다시 시작하겠다요?"
            )
